...
class TestDoCreate(unittest.TestCase):
    ...
    @patch('sys.stdout', new_callable=StringIO)
    def test_create_with_mixed_types_attributes(self, mock_stdout):
        """Test `do_create` with attributes of different types."""
        cmd = 'Place name="My_little_house" number_rooms=4 latitude=37.773972 longitude=-122.431297'
        HBNBCommand().do_create(cmd)
        # Insert specific checks here, e.g. check output or mock storage

    @patch('sys.stdout', new_callable=StringIO)
    def test_create_with_complex_string_attributes(self, mock_stdout):
        """Test `do_create` with complex string attributes, including spaces and escaped quotes."""
        cmd = 'Place name="\\\"My little house\\\""'
        HBNBCommand().do_create(cmd)
        # Insert specific checks here

    @patch('sys.stdout', new_callable=StringIO)
    def test_create_with_incomplete_attributes(self, mock_stdout):
        """Test `do_create` with incomplete attribute specifications."""
        HBNBCommand().do_create('User email=')
        self.assertIn("** attribute format error **", mock_stdout.getvalue())
